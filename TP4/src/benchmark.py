...
def main() -> None:
    ...
    if not ckpt_path.is_file():
        print(
            f"Erreur: checkpoint introuvable: {ckpt_path.resolve()}\n"
            "Lance d'abord l'entraînement (depuis la racine du dépôt) :\n"
            f"  python TP4/src/train.py --config TP4/configs/baseline_mlp.yaml --model mlp\n"
            f"  python TP4/src/train.py --config TP4/configs/gcn.yaml --model gcn\n"
            f"  python TP4/src/train.py --config TP4/configs/sage_sampling.yaml --model sage",
            file=sys.stderr,
        )
        sys.exit(1)
