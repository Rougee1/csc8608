...
def _neighbor_sampling_available() -> bool:
    """NeighborLoader nécessite pyg-lib ou torch-sparse (voir doc PyG)."""
    try:
...
def main() -> None:
    ...
    train_loader = None
    sage_full_batch = False
    if args.model == "sage":
        if _neighbor_sampling_available():
            ...
            print(
                "sage_mode: neighbor_sampling",
                f"batch_size={bs} num_neighbors=[{n1}, {n2}]",
            )
        else:
            sage_full_batch = True
            print(
                "sage_mode: full_batch (install pyg-lib or torch-sparse for NeighborLoader; "
                "Cora est petit, full-batch GraphSAGE reste valide pour le TP)"
            )
    ...
    for epoch in range(1, epochs + 1):
        ...
        if args.model in ["mlp", "gcn"] or sage_full_batch:
            with Timer() as t:
                if args.model == "mlp":
                    logits = model(x)
                else:
